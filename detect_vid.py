...
checkpoint = torch.load(checkpoint, weights_only=False, map_location=torch.device('cpu'))  # Fix applied here
...
def detect(original_image, min_score, max_overlap, top_k, suppress=None):
    ...
    for i in range(det_boxes.size(0)):
        ...
        # Text
        bbox = font.getbbox(det_labels[i].upper())  # Returns (left, top, right, bottom)
        text_width = bbox[2] - bbox[0]  # right - left
        text_height = bbox[3] - bbox[1]  # bottom - top
        text_size = (text_width, text_height)  # Mimic old getsize() behavior
